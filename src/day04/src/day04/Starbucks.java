...
public class Starbucks{
	...
	int[] arPrice;
	//신규 매장 등록하기
	//Cafe는 인터페이스이기 떄문에 외부에서 값을 전달받으려면
	//모든 추상메소드가 구현이 되어야 한다.
	//이를 이용하여, 신규 매장이라면 반드시 Cafe에 선언될 기능들을 구현해야 한다.
	//외부에서 기능이 모두 구현된 필드의 주소값이 C 객체로 전달되므로, 해당 필드에 접근하여
	//구현된 메소드를 사용해준다.
	public void register(Cafe c) {
		arMenu = c.getMenu();
		arPrice = c.getPrice();
		//만역 가격부분을 구현하지 않은 매장이라면 무료나눔 매장
		if(arPrice == null) {
			System.out.println("무료나눔 확인 완료");
			return;
		}
...
//			c.sell(arMenu[new Random().nextInt(arMenu.length)]);
			...
			System.out.println("판매방식 이상 없음");
		} catch (Exception e) {
			System.out.println("등록 실패 본사에 문의 해주세요");
			return;
